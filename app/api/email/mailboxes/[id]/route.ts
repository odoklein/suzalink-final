...
export async function PATCH(
  ...
  try {
    ...
    if (
      body.type !== undefined &&
      ["PERSONAL", "SHARED", "CLIENT", "CAMPAIGN"].includes(body.type)
    ) {
    ...
    if (typeof body.dailySendLimit === "number") {
      updateData.dailySendLimit = Math.max(
        1,
        Math.min(500, body.dailySendLimit),
      );
    }
    if (typeof body.warmupDailyLimit === "number") {
      updateData.warmupDailyLimit = Math.max(
        1,
        Math.min(100, body.warmupDailyLimit),
      );
    }
    if (typeof body.isActive === "boolean") {
      updateData.isActive = body.isActive;
    }
    if (body.trackingDomain !== undefined) {
      updateData.trackingDomain = body.trackingDomain?.trim() || null;
    }
