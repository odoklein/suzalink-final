...
// THREAD MANAGEMENT
// ============================================
// ... (skipping to CreateThreadRequest.broadcastAudience fix)
...
export async function createThread(
  ...
  if (request.channelType === "DIRECT") {
    ...
    channelId = await getOrCreateDirectChannel(
      creatorId,
  ...
  if (request.channelType === "DIRECT" && request.participantIds) {
    ...
    if (client) {
      ...
      // Combine all participant IDs (remove duplicates)
      participantIds = [
        ...new Set([creatorId, ...bdIds, ...clientUserIds, ...managerIds]),
      ];
    }
    ...
    if (campaign?.mission) {
      ...
      const bdIds = campaign.mission.client.bdAssignments.map(
        (a) => a.bdUserId,
      );
      ...
      // Combine all participant IDs (remove duplicates)
      participantIds = [
        ...new Set([creatorId, ...sdrIds, ...bdIds, ...managerIds]),
      ];
    }
    ...
    if (group) {
      ...
      participantIds = [...new Set([creatorId, ...memberIds])];
    }
  } else if (request.channelType === "BROADCAST") {
  ...
  const thread = await prisma.$transaction(async (tx) => {
    ...
    const newThread = await tx.commsThread.create({
      data: {
        ...
        broadcastAudience:
          request.broadcastAudience as unknown as Prisma.InputJsonValue,
        messageCount: 1,
  ...
  const isDirect =
    request.channelType === "DIRECT" ||
    request.subject.startsWith("Message avec ");
...
export async function getThread(
  ...
  return {
    ...
    messages: thread.messages.map((m) => ({
      ...
      readBy: (
        m as {
          readReceipts?: { user: { id: string; name: string }; readAt: Date }[];
        }
      ).readReceipts
...
export async function addAttachmentsToMessage(
  ...
  for (const f of files) {
    ...
    if (
      !storageService.isAllowedType(f.mimeType, COMMS_ATTACHMENT_ALLOWED_TYPES)
    ) {
...
export async function addOrRemoveReaction(
  ...
  userId: string,
  emoji: string,
): Promise<{ action: "added" | "removed"; threadId: string }> {
  const existing = await prisma.commsMessageReaction.findFirst({
  ...
  if (existing) {
    ...
    return { action: "removed", threadId: message.threadId };
  }

  await prisma.commsMessageReaction.create({
    data: { messageId, userId, emoji },
  });
  return { action: "added", threadId: message.threadId };
}
...
export async function deleteMessage(
  ...
  const canDelete =
    isManager ||
    (message.authorId === userId && message.createdAt >= fiveMinutesAgo);
